...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/export">
	    <xsl:text>Quantity, Package, Manufacturer, Mfg Part #</xsl:text>
	    <!-- all table entries -->
	...
	<xsl:template match="components">
		<xsl:for-each select="comp[count(. | key('partTypeByValueAndFootprint', concat(footprint, '-', value))[1]) = 1]">
			...
			<xsl:value-of select="fields/field[@name='LCSC_FP']"/>
			<xsl:text>","</xsl:text>
			<xsl:value-of select="fields/field[@name='Manufacturer']"/>
			<xsl:text>","</xsl:text>
			<xsl:value-of select="fields/field[@name='Mfg Part #']"/>
